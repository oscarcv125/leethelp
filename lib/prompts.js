...
const CONVERSATIONAL_START = /^(hi|hello|hey|yo|sup|thanks|thank you|ty|thx|ok|okay|kk|got it|makes sense|cool|nice|sure|yes|yep|no|nope|maybe|lol|hmm|wait|what|huh|sorry)\b/i;
...
function needsContext(query) {
  ...
  if (ALGO_KEYWORDS.test(q)) return true;
  if (CONVERSATIONAL_START.test(q)) return false;
  return q.length >= 30;
}
